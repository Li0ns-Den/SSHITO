...
def edit(dat,xmin,xmax,N_control,N):

    func = lambda x: x*0 + dat

    x = np.linspace(xmin,xmax,N_control)
    ...
    for i in np.arange(N_control):
        ...
        # Slider
        s = Slider(axamp, f'x = {x[i].round(2)}', 0, dat*10, valinit=yvals[i])
        sliders.append(s)
    ...
    axres = plt.axes([0.84, 0.8-((N_control)*0.05), 0.12, 0.02])
